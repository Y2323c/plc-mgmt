...
tab_session, tab_memo = st.tabs(["📝 セッション記録を追加", "💬 メモを追加"])
...
with tab_session:
    ...
    with st.form("session_form"):
        ...
        submitted_session = st.form_submit_button("追加", type="primary")
...
with tab_memo:
    ...
    with st.form("memo_form"):
        ...
        _coach_default_m = selected_ticket["coach_name"] if selected_ticket.get("coach_name") in COACH_LIST else (COACH_LIST[0] if COACH_LIST else None)
        coach_name_m = st.selectbox("コーチ", COACH_LIST, index=COACH_LIST.index(_coach_default_m) if _coach_default_m in COACH_LIST else 0)
        memo_note = st.text_area("メモ（気づき・観察・準備など）", height=150)
        submitted_memo = st.form_submit_button("追加", type="primary")
    ...
    if submitted_memo:
        if not memo_note:
            ...
            insert_record("coaching_logs", {
                ...
                "coach_name":        coach_name_m or None,
                "note":              memo_note,
                "created_at":        date.today().strftime(DATE_FMT_YMD),
            })
            st.session_state["_toast"] = "✓ メモを追加しました"
            st.rerun()
